...
const problemTypes = ["medical", "food", "shelter", "education", "water", "other"];
...
function ProblemSubmissionForm() {
  ...
  const validate = () => {
    ...
    if (!formData.location.trim()) nextErrors.location = t("location_required");
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      severityLabel: t(severityMeta[formData.severity].labelKey),
    ...
    try {
      // Mock API submit for Phase 1; replace with real API integration later.
      console.log("POST /api/needs", payload);
      await axios.post("/api/needs", payload).catch(() => Promise.resolve());
